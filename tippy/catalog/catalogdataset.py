...
class TIPCatalogDataset(Helper):
    def __init__(self, catalogs: list[TIPCatalog] = None):
        super().__init__()
    ...
    def _load_catalog_worker(self, args):
        ...
        try:
            ...
            catalog = TIPCatalog(path=catalog_file, catalog_type='all', load=True)
    ...
    def search_catalogs(self,
                        ...
                        n_proc: int = 16):
        if folder is None:
            folder = self.config['SCIDATA_DIR']
        folder = Path(folder)
            ...
            if not catalog.is_loaded:
                ...
                if load_result is False:
                    ...
                    if original_img is not None:
                        target_img = ScienceImage(original_img, telinfo = self.estimate_telinfo(original_img), load=True)
