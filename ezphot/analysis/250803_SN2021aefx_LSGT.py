...
databrowser.observatory = 'LSGT'
databrowser.objname = 'NGC1566'
databrowser.keys

#%% Load the data
target_imgset_fmt1 = databrowser.search(pattern='Calib*180.fits', return_type='science')
target_imgset_fmt2 = databrowser.search(pattern='aCalibrated*fit', return_type='science')
target_imglist =  target_imgset_fmt1.target_images + target_imgset_fmt2.target_images
#%%
...
circlemask1_kwargs = dict(
    x_position = 64.8877830,
    y_position = -54.8846364,
    radius_arcsec = 60,
    unit = 'deg',
    save = save,
    verbose = verbose,
...
def imgprocess(target_img):
    ...
    if do_circularmask:
        try:
            ...
            status['circular_mask1'] = True

        except Exception as e:
            status['circular_mask1'] = e
...
databrowser.objname = 'NGC1566'
# Stack the images
#imginfo_fmt1 = databrowser.search(pattern='Calib*180.fits', return_type='imginfo')
imginfo_fmt2 = databrowser.search(pattern='aCalibrated*.fit', return_type='imginfo')
imginfo_all = vstack([imginfo_fmt2])
...
for imginfo_group in imginfo_groups:
    imginfo_group = helper.group_table(imginfo_group, 'jd', 0.1)
...
def stackprocess(stack_img):
    ...
    stack_bkgrms.data = None
    gc.collect()
    return stacked_status, stack_img, stack_bkgrms
#%%
databrowser = DataBrowser('scidata')
databrowser.observatory = 'LSGT'
databrowser.objname = 'NGC1566'
databrowser.telkey = 'LSGT_SNUCAMII_1x1'
#stack_imgset = databrowser.search(pattern='Calib*180.fits', return_type='science')
stack_imgset = databrowser.search(pattern='aCalib*.fits', return_type='science')
...
catalogs = databrowser.search(pattern='*com.fits.cat', return_type='catalog')
catalogdataset = CatalogDataset(catalogs)
...
reference_singleimgset = databrowser.search(pattern='Calib*180.fits', return_type='science')
...
#%%
reference_singleimgset = ImageSet(reference_singleimglist_selected)
reference_singleimgset.run_ds9()
#reference_singleimgset.exclude_images(file_key = '20210828')
# %%
reference_singleimglist_selected = reference_singleimgset.target_images
reference_singlebkglist_selected = [Background(path = img.savepath.bkgpath, load = True) for img in reference_singleimgset.target_images]
reference_singlebkgrmslist_selected = [Errormap(path = img.savepath.bkgrmspath, emaptype = 'bkgrms', load = True) for img in reference_singleimgset.target_images] 
...
# %%
status, reference_img, reference_bkgrms = stackprocess(reference_img)
# %%
...
target_imgset = databrowser.search(pattern='*Calib*com.fits', return_type='science')

stacked_imglist = target_imgset.target_images
...
def subtract_process(target_img):
    try:
        reference_img = DIA.get_referenceframe_from_image(target_img,
                                                          overlap_threshold = 0.1)[0]
...
#%%
succeeded_imglist = []
for image in stacked_imglist:
    result = subtract_process(image)
    if hasattr(result, "_error"):  # failed image
        failed_images.append(result)
    else:  # successful
        succeeded_imglist.append(result)
#%%
failed_images1 = []
succeeded_imglist1 = []
for image in failed_images:
    result = subtract_process(image)
    if hasattr(result, "_error"):  # failed image
        failed_images1.append(result)
    else:  # successful
        succeeded_imglist1.append(result)
...
from ezphot.catalog import Catalog
from ezphot.catalog import CatalogDataset
catalogdataset_high = CatalogDataset()
catalogdataset_high.search_catalogs('NGC1566', 'sub*cat', folder = '/home/hhchoi1022/data/scidata/LSGT/LSGT_SNUCAMII_1x1')

catalogdataset_high.select_sources(ra = 64.9725, dec= -54.948081)
catalogdataset_all = CatalogDataset(catalogdataset_high.catalogs)
#%%
catalogdataset_kct = CatalogDataset()
catalogdataset_kct.search_catalogs('NGC1566', 'sub*cat', folder = '/home/hhchoi1022/data/scidata/KCT/KCT_STX16803_1x1')
catalogdataset_kct.select_sources(ra = 64.9725, dec= -54.948081)
catalogdataset_all = CatalogDataset(catalogdataset_all.catalogs + catalogdataset_kct.catalogs)
#%%
catalogdataset_high = CatalogDataset()
catalogdataset_merge = CatalogDataset()
catalogdataset_high.search_catalogs('NGC1566', 'sub*cat', folder = '/home/hhchoi1022/data/scidata/RASA36/RASA36_KL4040_HIGH_1x1')
catalogdataset_merge.search_catalogs('NGC1566', 'sub*cat', folder = '/home/hhchoi1022/data/scidata/RASA36/RASA36_KL4040_MERGE_1x1')

catalogdataset_high.select_sources(ra = 64.9725, dec= -54.948081)
catalogdataset_merge.select_sources(ra = 64.9725, dec= -54.948081)
catalogdataset_all = CatalogDataset(catalogdataset_all.catalogs + catalogdataset_high.catalogs + catalogdataset_merge.catalogs)
# %%
# catalogdataset_all.select_catalogs(filter = 'g')
lc = LightCurve(catalogdataset_all)
lc.extract_source_info(ra = 64.9729, dec= -54.948381)

# %%
lc.plt_params.figure_figsize = (10, 6)
lc.plt_params.xlim= [59500, 59700]
lc.plt_params.ylim = [16.5, 13.5]
lc.plot(ra = 64.9729, dec= -54.948381, matching_radius_arcsec = 10, color_key = 'filter')
