...
class TIPPSFPhotometry(Helper):
    #TODO SHOW WHEN VISUALIZE = TRUE, SAVEFIG ONLY SAVE_FIG = True
    def __init__(self):
        super().__init__()
    ...
    def select_psfstars(self,
        ...
        def select_subset(sources: Table, visualize: bool = False, verbose = True) -> Table:
            ...
            self.print(f'Initial sources: {len(filtered_catalog)}', verbose)
            ...
            if x_key not in filtered_catalog.keys() or y_key not in filtered_catalog.keys():
                ...
                self.print(f'[INNERREGION CUT] {len(filtered_catalog)} sources passed within X = [{x_inner_min},{x_inner_max}], Y = [{y_inner_min},{y_inner_max}]', verbose)
            
            _plot_if_visualize(filtered_catalog[mag_key], filtered_catalog[fwhm_key], 'r', label = 'InnerRegion cut', alpha = 0.3)

            # Step 2: Isolation
            if x_key not in filtered_catalog.keys() or y_key not in filtered_catalog.keys():
                print(f"Warning: '{x_key}' or '{y_key}' not found in sources.")
            else:
                ...
                self.print(f'[ISOLATION CUT] {len(filtered_catalog)} sources passed with isolation radius {isolation_radius} pixels', verbose)
            ...
            if flux_key not in filtered_catalog.keys():
                print(f"Warning: '{flux_key}' not found in sources.")
            else:
                filtered_catalog = filtered_catalog[(filtered_catalog[flux_key] / filtered_catalog[fluxerr_key]) > snr_lower]
                self.print(f"[SNR CUT]: {len(filtered_catalog)} sources passed with SNR > {snr_lower}", verbose)
            ...
            if fwhm_key not in filtered_catalog.keys():
                print(f"Warning: '{fwhm_key}' not found in sources.")
            else:
            ...
            if eccentricity_key not in filtered_catalog.keys():
                print(f"Warning: '{eccentricity_key}' not found in sources.")
            else:
                ...
                self.print(f"[ECCENTRICITY CUT]: {len(filtered_catalog)} passed eccentricity < {eccentricity_upper} and within ±{eccentricity_sigma} sigma of median ({eccen_median:.2f} ± {eccen_std:.2f})", verbose)
        ...
        for i in range(num_grids):
            for j in range(num_grids):
                ...
                self.print(f"Grid ({i},{j}): {len(sub_tbl)} candidates, {len(result) if result is not None else 0} selected.", verbose)

        if len(selected_catalog) == 0:
            if verbose:
                print("No suitable stars found in any grid.")
            return Table()

        result_tbl = Table(np.hstack(selected_catalog))

        self.print(f"Selected {len(result_tbl)} stars from {len(sources)} candidates (grid mode).", verbose)
    ...
    def extract_sources(self,
                        ...
                        **kwargs) -> Table:
        """
        Detect stars using SEP (with background RMS) or DAOStarFinder.
        
        Parameters
        ----------
        method : str
            'sep' (default, supports RMS map) or 'dao' (Gaussian matched filter)
        ...
        self.print('Detected {} sources'.format(len(new_objects)), verbose)       
    ...
    def build_epsf_model_psfex(self,
        ...
        target_img : ScienceImage or ReferenceImage
            Image to run PSFEx on.
        num_grids : int
            Number of grids along one axis (e.g., 3 for 3x3).
        fwhm_estimate_pixel : float
            Estimated FWHM of stars in pixels.
        oversampling : int
            Oversampling factor for subpixel modeling.
        """
    ...
    def psf_photometry(self,
                    ...
                    **kwargs):
        """
        Perform PSF photometry on the given image using EPSFModel(s) with aperture correction.

        Parameters
        ----------
        epsf_model : EPSFModel or dict
            Either a single global EPSFModel or a dict of grid-based EPSFModels.
        apply_aperture_correction : bool, optional
            If True, apply aperture correction to PSF photometry results.
            The correction is based on the ratio between aperture flux (FLUX_APER) 
            and PSF-fitted flux (FLUX_PSF).
        """
        ...
        def cut_edge_sources(sources, edge_buffer=100):
            ...
            self.print(f'{len(sources[~edge_mask])} sources filtered out due to edge mask.', verbose)
        ...
        # PSF Photometry
        self.print('Starting PSF photometry...', verbose)
        ...
        def run_psf_photometry(data, sources):
            ...
            if isinstance(epsf_model_dict, EPSFModel):
                self.print('Using global EPSF model for photometry.', verbose)
                ...
                self.print('Start generating model images...', verbose)
                ...
                self.print('Start generating residual images...', verbose)
                ...
                self.print('Using grid-based EPSF models for photometry.', verbose)
                h, w = data.shape
                ...
                for (i, j), epsf_model in epsf_model_dict.items():
                    self.print(f'Processing grid ({i},{j}) with EPSF model shape {epsf_model.data.shape}', verbose)
                    x_min, x_max = j * x_step, (j + 1) * x_step
                    y_min, y_max = i * y_step, (i + 1) * y_step
        ...
        if apply_aperture_correction and 'FLUX_APER' in result.colnames:
            self.print('Applying aperture correction...', verbose)
            ...
            if len(good_stars) > 10:
                ...
                aper_correction = False
                self.print('No aperture correction applied due to lack of the good stars', verbose)
        ...
        if n_iterations > 0:
            ...
            for i in range(n_iterations):
                self.print(f'Running {i+1}/{n_iterations} iterations of PSF photometry...', verbose)
                ...
                new_matched, prev_matched, new_unmatched = self.cross_match(new_coord, previous_coord, max_distance_second = separation_in_arcsec)
                sources_iter = sources_iter[new_unmatched]
                self.print(f'{len(new_matched)} are filtered out due to cross-match with small separation.', verbose)
                
                # Check if sources are within the image bounds
                filtered_sources_iter = cut_edge_sources(sources_iter, edge_buffer=100)
                self.print(f'{len(filtered_sources_iter)} source are detected in iteration {i+1}.', verbose)
                if visualize:
