...
#%% Load the data
#sdtdata.sync_scidata(targetname = 'T02386')
target_imgset = databrowser.search(pattern='calib*100.fits', return_type='science')
...
def imgprocess(target_img):
    ...
    if do_photometric_calibration:
        if not do_aperturephotometry:
            sex_catalog = target_img.catalog
        try:
...
#%% Check photometry on single images
from ezphot.catalog import CatalogDataset
ra  = 79.9015969
dec = -60.8011349
catalogs = CatalogDataset().search_catalogs(
    target_name = 'T02386',
    search_key = '.fits.cat'
)
catalog_dataset = CatalogDataset(catalogs[0])
catalog_dataset.select_sources(ra=ra, dec=dec, radius = 10)
#%%
#%%
#lc.extract_source_info(ra, dec)
#%%
from ezphot.dataobjects import LightCurve
filterlist = ['m400', 'm425', 'm450', 'm475', 'm500', 'm525', 'm550', 'm575', 'm600', 'm625', 'm650', 'm675', 'm700']
catalog_dataset.select_catalogs( seeing = 4, depth = 18.5)
lc = LightCurve(catalog_dataset)
lc.plt_params.figure_figsize = (10, 6)
#lc.plt_params.ylim = [16.5, 17.8]
lc.plt_params.xlim = [60750, 60880]
lc.plot(ra, dec, 5, 'MAGSKY_APER_3', fluxerr_key = 'MAGERR_APER_3')
#%%
from ezphot.dataobjects import PhotometricSpectrum
filterlist = ['m400', 'm425', 'm450', 'm475', 'm500', 'm525', 'm550', 'm575', 'm600', 'm625', 'm650', 'm675', 'm700']
catalog_dataset.select_catalogs(filter =filterlist, obs_start = '2025-04-08T12:00:00', obs_end = '2025-04-09T12:00:00')
spec = PhotometricSpectrum(catalog_dataset)
#%%
spec.plot(ra, dec, 5, 'MAGSKY_AUTO', fluxerr_key = 'MAGERR_AUTO', color_key = 'obsdate')
#%%
...
#%%
selected_imglist = Stacking.select_quality_images(target_imglist, seeing_limit = 8, depth_limit = 15, ellipticity_limit = 1.0, max_numbers = len(target_imglist), visualize = True)
#%%
...
databrowser.objname = 'T02386'
databrowser.keys
stack_imgset = databrowser.search(pattern='calib*100.com.fits', return_type='science')
...
def process_batch(batch_images, batch_index, max_workers=16):
    ...
    results = []
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(stackprocess, img) for img in batch_images]
...
for batch_index, batch in enumerate(chunk_list(stack_imglist, batch_size)):
    batch_results = process_batch(batch, batch_index, max_workers=8)
    all_results.extend(batch_results)
# %%
from ezphot.catalog import CatalogDataset
...
catalog_dataset = TIPCatalogDataset(catalogs[0])
catalog_dataset.select_sources(ra=ra, dec=dec, radius = 10)
#%%
from ezphot.dataobjects import LightCurve
filterlist = ['m400', 'm500', 'm600', 'm700', 'm800', 'g', 'r', 'i']
catalog_dataset.select_catalogs(seeing=4, filter=filterlist)
lc = LightCurve(catalog_dataset)
lc.plt_params.figure_figsize = (10, 6)
lc.plt_params.ylim = [18, 16]
lc.plt_params.xlim = [60750, 60880]
lc.plot(ra, dec, 5, 'MAGSKY_APER_3', fluxerr_key='MAGERR_APER_3')
#%%
from ezphot.dataobjects import PhotometricSpectrum
filterlist = ['m400', 'm425', 'm450', 'm475', 'm500', 'm525', 'm550', 'm575', 'm600', 'm625', 'm650', 'm675', 'm700','m725','m750','m775','m800','m825','m850','m875']
# catalog_dataset.select_catalogs(filter=filterlist, obs_start='2025-04-20T12:00:00', obs_end='2025-04-30T12:00:00')
catalog_dataset.select_catalogs(filter=filterlist, obs_start='2025-07-01T12:00:00', obs_end='2025-07-15T12:00:00')
spec = PhotometricSpectrum(catalog_dataset)
spec.OFFSET = 0.0
#%%
spec.plt_params.ylim = [18, 15]
spec.plot(ra, dec, 1, 'MAGSKY_APER_3', fluxerr_key = 'MAGERR_APER_3', color_key = 'obsdate')
#%%
# Additional analysis or processing can be added here if needed.
