...
for target_img in target_imglist:
    if 'FLXSCALE' in target_img.header:
        target_img.header.remove('FLXSCALE')
        target_img.write()
        print(f"Removed FLXSCALE from {target_img.path}")
...
imginfo_all['mjd'] = Time(imginfo_all['obsdate']).mjd
...
# cutout_reference_img = reference_img.cutout(x = 64.9725, y = -54.948081, size = 2000, coord_type = 'coord')
#%%
failed_images = []
successful_results = []
...
def subtract_process(target_img):
    try:
        reference_img_result = target_img.get_referenceframe(overlap_threshold = 0)
        if reference_img_result is None:
            reference_img_result = target_img.get_referenceframe(max_obsdate = '2025-09-22')
        result = DIA.find_transients(
            target_img=target_img, 
            reference_imglist=[reference_img_result[0]],
            target_bkg=None,
            detection_sigma = 5,
            aperture_diameter_arcsec = [5,7,10],
            aperture_diameter_seeing = [3.5,4.5],
