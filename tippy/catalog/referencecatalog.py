...
class ReferenceCatalog(Helper):
    ...
    def __repr__(self):
        txt = f"ReferenceCatalog[objname = {self.objname}, type = {self.catalog_type}]"
        return txt
    ...
    def query_catalog(self, catalog_name: str = 'APASS', verbose: bool = False):
    ...
    def _update_history(self):
        self.history = ReferenceCatalogHistory(objname = self.objname, ra = self.ra, dec = self.dec, fov_ra = self.fov_ra,fov_dec = self.fov_dec, filename = self.filename, cat_type = self.catalog_type, save_date = self.save_date)
