...
from tippy.image import ScienceImage, CalibrationImage, MasterImage, ReferenceImage, Errormap, Background
from tippy.photometry import TIPProjection
from tippy.photometry import TIPBackground
from tippy.photometry import TIPPSFPhotometry
import re
...
projection_handler = TIPProjection()
...
class TIPStacking(Helper):
    ...
    def stack_multiprocess(self,
        ...
        if clip_type is 'extrema':
            if len(image_datalist) - nlow - nhigh < 3:
                self.print(f"[Combiner] Not enough images to clip: ({len(image_datalist)}). Clip type is set as None", verbose)
                clip_type = None
    ...
    def stack_swarp(self,
        ...
        # Image combine
        self.print(f"Start image combining...", verbose)
        imagestack_path = None
        weightstack_path = None
        # Run swarp 
        stack_pathlist = self.run_swarp(
        ...
        if target_errormaplist is not None:
            self.print(f"Start weight combining...", verbose)
            stack_pathlist = self.run_swarp(
                target_path = image_pathlist,
                swarp_configfile = target_imglist[0].config['SWARP_CONFIG'],
                swarp_params = None,
        ...
        if type(target_imglist[0]) == CalibrationImage:
            ...
            stack_instance.header = self.merge_header(stack_instance.header, combined_header, exclude_keys = ['PV*'])
        else:
        ...
        stack_weight_instance.header = self.merge_header(stack_weight_instance.header, combined_header, exclude_keys = ['PV*'])
        ...
        if save:
            ...
            self.print(f"Stacked image saved to {stack_instance.path}", verbose)
            self.print(f"Stacked weight map saved to {stack_weight_instance.path}", verbose)
        else:
            stack_instance.load()
    ...
    def match_seeing(self,
        ...
        method : str
            Method to determine reference seeing ('min', 'max', or 'median')
        seeing_key : str
        ...
        self.print(f"Matching seeing to reference FWHM = {ref_seeing:.3f} using {kernel} method", verbose)
        ...
        for idx, (target_img, target_errormap) in enumerate(iterator):
            ...
            if seeing_diff < tolerance:
                ...
                self.print(f"Skipping convolution for {target_img.path.name} (already matches reference seeing with the tolerenace {tolerance}arcsec) [diff = {seeing_diff}arcsec]", verbose)
                continue
            
            # Convolve the target image
            target_bkg = target_bkglist[idx] if target_bkglist is not None else None
            if kernel.lower() == 'gaussian':
                convolved_data, updated_header = self.img_convolve(
...
# %%
if __name__ == '__main__':
    from tippy.photometry import TIPErrormap
    emap = TIPErrormap()
    calib_imagelist = []
