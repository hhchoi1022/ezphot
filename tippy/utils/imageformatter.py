...
class ImageFormatter(TIPDataBrowser):
    ...
    def search_files(self, pattern='*.fits'):
        # key = filter, value = list
        self.target_pathdict = self.search(pattern=pattern, return_type='path')
...
if __name__ == '__main__':
    ...
    formatter.observatory = 'LSGT'
    formatter.telkey = 'LSGT_SNUCAMII_1x1'
    formatter.objname = 'NGC1566'
    formatter.filter = 'g'
    formatter.search_files(pattern='*.fit')
    formatter.check_header(modify_header=True, TELESCOP='LSGT', IMGTYPE = 'LIGHT', FILTER = 'g')
#%%