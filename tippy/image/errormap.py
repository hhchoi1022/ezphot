...
class Errormap(DummyImage):
    ...
    def to_rms(self):
        """
        Convert the Errormap to a Background image.
        If bkgpath is provided, it will save the Background image to that path.
        """
        if self.data is None:
            raise ValueError("Cannot convert Errormap to Background: data is not registered.")
    ...
    def to_weight(self):
        ...
        If maskpath is provided, it will save the Weight image to that path.
        """
        if self.data is None:
            raise ValueError("Cannot convert Errormap to Weight: data is not registered.")
...
if __name__ == '__main__':
    target_img = Path('/data/data1/factory_hhchoi/data/scidata/7DT/7DT_C361K_HIGH_1x1/T22956/7DT16/g/calib_7DT16_T22956_20250329_043218_g_100.fits.errormap')
    self = Errormap(target_img, load=True)
